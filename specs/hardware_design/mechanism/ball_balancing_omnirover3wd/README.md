...
| モータマウント | MotorMount_v3（3Dプリント） | 3 | フェース板3.6 mm厚（s = -20.5〜-16.9）、M3×4＋中央逃げØ9、Ø5.8×2.5ザグリ、立ち上がりアーム＋水平フット M4×4でL1下面へ締結 | `cad/parts/MotorMount_v3.step/.stl` |
...
Fusion 360 プロジェクト `nenecchi/ball-balancing-omni-rover` 内の `ball_balancing_omnirover3wd_assembly`。31オカレンス、37アズビルトジョイント（リジッド34、ホイール↔アダプタのリボリュート3）。エクスポートは `cad/ball_balancing_omnirover3wd_assembly.f3d` / `.step`、個別部品は `cad/parts/`（STEP＋STL）。
...
- MotorMount_v3 はフェース板3.6 mm（ホイール側を向く）＋立ち上がりアーム＋水平フット（M4×4、r = 57/64、y = ±9）。M3ねじ頭はフェース板ホイール側のØ5.8×2.5 mmザグリに沈め（残り座ぐり底約0.6 mm）、ホイール面との隙間を確保する。アームはホイールリムを回避する経路（車軸方向座標 q ≈ -12.5 で半径 p ≥ 26 mm）を取る。
