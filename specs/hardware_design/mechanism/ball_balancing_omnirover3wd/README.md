...
車軸方向は接触法線回りに $\phi$ だけ回転させた $a'_i$ とする。$\phi=0$ の同軸配置では3基のモータが車軸交点 $(0,0,140.3\ \mathrm{mm})$ 付近で衝突するため、モータを内向きに収める目的で $\phi=65^\circ$ とし、車軸を内向き・上方・接線方向へ傾けたピンホイール配置とした。

$$
{}^Ba'_i=\cos\phi\,{}^Ba_i+\sin\phi\,{}^Bt_i
$$
...
| シャシー外接半径（最大外形） | 80.0 mm |
| モータ先端の最大高さ（L1下面との隙間） | 142.0 mm（5.0 mm） |
| モータ先端の外接半径 | 73.6 mm |
...
| モータマウント | MotorMount_v2（3Dプリント） | 3 | フェース板4 mm厚、M3×4＋中央逃げØ9、ガセット付きLアーム、M4×4フットでL1下面へ締結 | `cad/parts/MotorMount_v2.step/.stl` |
| ホイールアダプタ | WheelAdapter_v2（3Dプリント） | 3 | Ø11.9スリーブ＋Ø8.4ネックの段付きブッシュ、ボアØ4.15（FIT0521軸Ø4→ホイールボアØ12変換） | `cad/parts/WheelAdapter_v2.step/.stl` |
...
| M-09 | ホイール、締結部、配線を含む最大外形 | 直径160 mm以内 |
...
Fusion 360 プロジェクト `nenecchi/ball-balancing-omni-rover` 内の `ball_balancing_omnirover3wd_assembly`。29オカレンス、37アズビルトジョイント（リジッド34、ホイール↔アダプタのリボリュート3）。エクスポートは `cad/ball_balancing_omnirover3wd_assembly.f3d` / `.step`、個別部品は `cad/parts/`（STEP＋STL）。
...
| ホイール中心 $F_i$ | 水平半径 42.4、z = 110.6、方位 0/120/240 deg |
| 車軸 $a'_i$ | $\phi=65^\circ$（内向き・上方・接線） |
| モータ | フェース $s=18.5$ mm、先端 z ≈ 142、機体内部に収容 |
...
- MotorMount_v2 はフェース板4 mm＋ガセット一体化アーム＋水平フット（4 mm厚、M4×4）。M3ねじ頭はフェース板のØ5.8×2.7 mmザグリに沈め、ホイール面との隙間を確保する。
- WheelAdapter_v2 はØ11.9/Ø8.4段付きブッシュ。Ø11.9部はホイールボアØ12全幅に係合し、Ø8.4ネックはマウント中央穴を貫通する。ボアØ4.15はモータ軸Ø4を受ける。
- 推奨積層方向：マウントはフェース板をベッド面に、アダプタは軸を立てて印刷する。
- 穴は印刷後のリーマ・ドリル通しを想定しM3→Ø3.4、M4→Ø4.5、ボアØ4.15とした。

### 検証結果

- 全406ペアの干渉チェック：干渉体積 0 mm³
- 最大外形：半径 80.0 mm（Ø160、M-09適合）。外縁はフレーム板のみ
- 全高：256 mm
- モータ先端–L1下面の隙間：約5 mm。モータ先端外接半径 73.6 mm
- ホイールボアはホイール中心を通る車軸上にあり、接触緯度 $\lambda=55^\circ$ の接触点配置を維持

## 機構変更の影響

FIT0521は従来サーボと外形、出力軸、取付方法が異なるため、既存のRotramaサーボマウント用CADは製作用として使用しない。モータは機体内部へ内向きに配置し、車軸を接触法線回りに $\phi=65^\circ$ 回転させたピンホイール配置とした。これにより車軸交点でのモータ同士の衝突を回避しつつ、ボールへの接触幾何（$\lambda=55^\circ$、ホイール中心位置）は従来設計と同一を維持する。なお、車軸回転により各ホイールの駆動方向ベクトルは水平接線方向から $\cos\phi\,t_i-\sin\phi\,a_i$ 方向へ回転するため、制御則の車輪速度–ボール角速度マッピングはこの回転を反映させる必要がある。
