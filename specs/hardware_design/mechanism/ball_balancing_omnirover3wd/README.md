...
FIT0521向けのモーター固定パーツとフレーム1層プレートは `cad/fit0521_motor_mount.py` で生成する(同軸・外側張り出し配置。詳細は `cad/fit0521_motor_mount_report.md`)。この配置では最大外形が約Ø204 mmとなりM-09を超過するため、外形制約が優先される場合は配置を再検討する。
