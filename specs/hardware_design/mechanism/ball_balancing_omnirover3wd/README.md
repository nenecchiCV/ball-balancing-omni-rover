...
| モータマウント | MotorMount_v7（3Dプリント） | 3 | 最小面数の解析形状（49面・単一ボディ）。車軸同軸の厚肉円柱ブロック＋垂直アーム＋水平フット板のプリミティブ構成。ギアボックス挿入ポケットØ28＋平坦座面（Ø25ギアボックスを座面まで収容）、中央Ø9シャフト／アダプタ通路。モータ取付4×M3インサート穴（ボルト円Ø17、ホイール側Ø4.5×5.5ポケット＋Ø3.4パイロット）。フレームL1との締結のみ水平フット板の4×Ø4.3貫通穴（r=57/64、y=±9）でナット＋ボルト式。キャスター座はブロック外面のØ20リセス＋Ø3.8×6 mmインサート穴×2 | `cad/parts/MotorMount_v7.step/.stl` |
| ホイールアダプタ | WheelAdapter_v4（3Dプリント） | 3 | Ø11.9スリーブ（ホイールボアØ12に全幅係合）、ボアØ4.1（FIT0521軸Ø4→ホイールボアØ12変換）、マウント貫通部はØ8.0ストレートチューブ（Ø9通路へ半径0.5 mm隙間、モータ軸係合約7 mm） | `cad/parts/WheelAdapter_v4.step/.stl` |
| ボールキャスター | Pololu 3/8インチボールキャスター（金属球・成形ハウジング） | 3 | 球Ø9.525 mm、フランジ取付穴Ø2.29（中心から±6.73 mmピッチ）、球心は取付面から+5.31 mm、マウントパッドへM2.5熱圧入インサート＋小ねじ締結、球心–ボール心距離55.7 mm（ボール面に約0.3–0.4 mmの転がり隙間） | `cad/parts/pololu_ball_caster_375.step`、[寸法図](pololu_ball_caster_375_dim.pdf) |
...
Fusion 360 プロジェクト `nenecchi/ball-balancing-omni-rover` 内の `ball_balancing_omnirover3wd_assembly`。32オカレンス。フレーム・モータ・ホイール間はリジッドおよびリボリュートのアズビルトジョイントで拘束し、モータ3個・購入品ボールキャスター3個・MotorMount_v7の3オカレンスは数値検証済みの配置で接地固定。エクスポートは `cad/ball_balancing_omnirover3wd_assembly.f3d` / `.step`、個別部品は `cad/parts/`（STEP＋STL）。
...
| ボールキャスター球 Ø9.525 | 球心：水平半径 32.0、z = 95.6（ボール心距離55.7 mm、方位0/120/240 deg）。フランジはマウントのキャスターパッドに締結 |
...
- MotorMount_v7 は面数最小化を目的に、車軸と同軸の円柱ブロック・垂直アーム・水平フット板・各種円柱切削のみで構成した解析形状（49面・単一ボディ・体積約18.2 cm³）とした。板状フェースを廃止しギアボックスØ25を座面まで挿入する厚肉ブロックは踏襲（積層3Dプリントでの反り・歪み対策）。モータは前面M3タップ面をブロック内の平坦座面へ着座し、ギアボックス外周にはポケット壁を残す。座面中央はØ9のシャフト／アダプタ通路でホイール側へ開放する。
- 締結仕様：フレームL1との固定のみ水平フット板（L1下面と平行な板、z = 143–147）の4×Ø4.3貫通穴（r = 57/64、y = ±9、L1のØ4.2穴に整合）でM4ナット＋ボルト式とし、それ以外のねじ穴はすべて熱圧入インサート前提とした。モータ取付はホイール側Ø4.5×5.5ポケットへM3インサートを圧入し、Ø3.4パイロット経由でモータM3タップへ締結する（ボルト円Ø17×4）。キャスター取付はØ3.8×6 mm穴へM2.5インサートを圧入しM2.5小ねじで締結する。
- キャスター座はブロック外面に切削したØ20リセスで、Pololu 3/8インチキャスターのフランジ面（実測約Ø16 mm）を全周で受ける。取付穴Ø2.29（±6.73 mmピッチ）に対応するØ3.8インサート穴×2を持ち、締結はM2.5熱圧入インサート＋小ねじを想定。
- WheelAdapter_v4 はØ11.9スリーブ（ホイールボアØ12に全幅係合）＋ボアØ4.1。マウントを貫通する区間はØ8.0ストレートチューブとし、Ø9通路に半径0.5 mmの隙間で通る。モータ軸の係合長は約7 mm。
- 推奨積層方向：マウントは側面寝かせ（-Y側面をベッド面に）で印刷する。この向きではブロック座面・アーム・フット板の主面がすべて垂直壁となり、ギアボックスポケット・インサート穴も横向きのためサポートなしで印刷できる。アダプタは軸を立てて印刷する。
- 穴は印刷後のリーマ・ドリル通しを想定しM3パイロット→Ø3.4、M4（ナット＋ボルト式）→Ø4.3、ボアØ4.1とした。インサート穴Ø4.5（M3用）・Ø3.8（M2.5用）はインサート品種に応じて微調整する。

### 検証結果

- 全ポッド×全ペアのワールド空間干渉チェック：実質干渉なし（モータ・アダプタ・ホイール・キャスター・ボール・フレームL1/L2すべて0）。残存はキャスターフランジ–リセス座面・キャスター球–ボールの約3 mm³相当の境界接触のみ
- 最大外形：半径 107.6 mm（直径215 mm、M-09適合）。支配するのはモータ先端角部
- 全高：約257 mm
- ホイール–ボール：車軸中心誤差 0.00 mm、トレッド面がボールに接線接触（$\lambda=65^\circ$）
- キャスター球–ボール：球心–ボール心距離 55.7 mm（Ø9.525球、ボール面に約0.3–0.4 mmの転がり隙間、方位0/120/240 deg）
- 隣接ホイール間クリアランス：約4.7 mm
- モータギアボックスØ25：Ø28ポケットへ挿入しノーズ面を平坦座面へ着座。4×M3インサート穴がモータM3タップ（ボルト円Ø17）と一致
- フレームL1締結：水平フット板の4×Ø4.3貫通穴がL1のØ4.2穴（r = 57/64、y = ±9）と一致、L1下面に密着
- モータ軸–アダプタボア係合長：約7 mm。アダプタチューブØ8.0がマウントØ9通路を半径0.5 mm隙間で貫通
...
FIT0521向けのモーター固定パーツ・フレーム1層プレート・ボールキャスターホルダーは `cad/fit0521_motor_mount.py` で生成する(同軸・外側張り出し配置、キャスターは球緯度+30°×3個。詳細は `cad/fit0521_motor_mount_report.md`)。この配置では最大外形が約Ø204 mmとなりM-09を超過するため、外形制約が優先される場合は配置を再検討する。
