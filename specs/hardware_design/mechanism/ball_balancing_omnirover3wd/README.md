...
| モータマウント | MotorMount_v4（3Dプリント） | 3 | フェース板3.6 mm厚（s = -20.5〜-16.9）、M3×4＋中央逃げØ9、Ø5.8×2.5ザグリ、立ち上がりアーム＋水平フット M4×4でL1下面へ締結、ボールキャスターソケット一体 | `cad/parts/MotorMount_v4.step/.stl` |
...
| キャスター球 | CasterBall_10（Ø10 POM/スチール球） | 3 | マウント一体ソケットに圧入、ボール面を緯度約37 degで全方向転がり支持 | `cad/parts/CasterBall_10.step/.stl` |
...
Fusion 360 プロジェクト `nenecchi/ball-balancing-omni-rover` 内の `ball_balancing_omnirover3wd_assembly`。34オカレンス、40アズビルトジョイント（リジッド37、ホイール↔アダプタのリボリュート3）。キャスター球↔マウントはリジッドジョイントで位置表現（実機はソケット内で全方向自由転動）。エクスポートは `cad/ball_balancing_omnirover3wd_assembly.f3d` / `.step`、個別部品は `cad/parts/`（STEP＋STL）。
...
| キャスター球 Ø10 | 中心 $s=-26$ mm・半径方向26 mm、ボール接点は緯度約37 deg（各ポッド対称面上） |
...
- MotorMount_v4 はフェース板3.6 mm（ホイール側を向く）＋立ち上がりアーム＋水平フット（M4×4、r = 57/64、y = ±9）＋ボールキャスターソケット一体ブロック。M3ねじ頭はフェース板ホイール側のØ5.8×2.5 mmザグリに沈め（残り座ぐり底約0.6 mm）、ホイール面との隙間を確保する。アームはホイールリムを回避する経路（車軸方向座標 q ≈ -12.5 で半径 p ≥ 26 mm）を取る。
- ソケットは板下端から張り出す一体ブロック（±7 mm厚）。球ポケットØ10.6、口部はボール側へØ8.7で球を捕捉し、球先はボール面に約2 mm突出して接線接触する。球は口部の弾性変形で圧入（POM/スチール球Ø10を想定）。
...
- 全ペアのワールド空間干渉チェック：干渉体積 0 mm³（ジョイント作成後に再確認済み）
- 最大外形：半径 107.6 mm（直径215 mm、M-09適合）。支配するのはモータ先端角部
- 全高：約257 mm
- ホイール–ボール：車軸中心誤差 0.00 mm、トレッド面がボールに接線接触（$\lambda=65^\circ$）
- キャスター球–ボール：球心–ボール心距離 55.00 mm（Ø10球がボール面に接線接触、緯度約37 deg）
- 隣接ホイール間クリアランス：約4.7 mm
- ソケットブロック–モータ胴体：約2.4 mm、–ホイール：約2.9 mm、–ボール（口部リップ）：約2 mm
...
各モータポッドはL1への片持ち締結に加え、マウント一体のボールキャスターソケット（Ø10球）でボール面を緯度約37 degの第2接点として支持する。これによりポッド自重とモータ片持ちモーメントの一部がフレームではなくボールへ直接伝わり、フット締結部とモータ軸の負荷を軽減するとともに、ホイール法線荷重が増して駆動トラクションが向上する。キャスターは全方向に自由転動する受動支持であり、駆動自由度を拘束しない。
