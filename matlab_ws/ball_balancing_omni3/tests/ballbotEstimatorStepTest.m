classdef ballbotEstimatorStepTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testInterfaceAndControllerRegression(testCase)
            ...
            controllerOutput = ballbotLqiControllerUpdate(estimate, ...
                zeros(3, 1), zeros(2, 1), true, false, diagnostics, ...
                zeros(3, 1), p);
            motorOutput = ballbotSpeedPiMotorStep( ...
                controllerOutput(1:3), zeros(3, 1), zeros(3, 1), ...
                controllerOutput(6), p);
            ...
            testCase.verifyEqual(estimate(1:13), zeros(13, 1), ...
                AbsTol=1.0e-10);
            testCase.verifyGreaterThan(estimate(14), 0.99);
            testCase.verifyEqual(motorOutput(1:3), zeros(3, 1), ...
                AbsTol=1.0e-12);
            testCase.verifyEqual(uint8(controllerOutput(6)), uint8(1));
        end
        ...
        function testControllerInhibitsYawUntilBiasIsReady(testCase)
            ...
            outputNotReady = ballbotLqiControllerUpdate(estimate, ...
                zeros(3, 1), zeros(2, 1), true, false, ...
                biasDiagnostics, wheelRate, p);
            outputReady = ballbotLqiControllerUpdate(estimate, ...
                zeros(3, 1), zeros(2, 1), true, true, ...
                biasDiagnostics, wheelRate, p);
            outputSmallYaw = ballbotLqiControllerUpdate(estimate, ...
                [0; 0; 0.5*p.controller.yawBiasCommandBypassThreshold], ...
                zeros(2, 1), true, false, biasDiagnostics, wheelRate, p);
            outputCommanded = ballbotLqiControllerUpdate(estimate, ...
                [0; 0; 0.2], zeros(2, 1), true, false, ...
                biasDiagnostics, wheelRate, p);
            ...
            testCase.verifyEqual(outputSmallYaw(1:3), ...
                outputNotReady(1:3), AbsTol=1.0e-12);
            testCase.verifyGreaterThan(norm( ...
                outputCommanded(1:3) - outputNotReady(1:3)), 0);
